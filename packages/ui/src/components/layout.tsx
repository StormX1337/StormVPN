...
export function EmptyState({ icon, title, description, action, className }: { icon?: React.ReactNode; title: string; description?: string; action?: React.ReactNode; className?: string }) {
  return (
    <div className={cn('flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed px-6 py-12 text-center', className)}>
      {icon ? <div className="rounded-full bg-muted p-3 text-muted-foreground [&_svg]:size-5">{icon}</div> : null}
      <div className="space-y-1">
 ...
 */
export function StatTile({ label, value, hint, icon, className }: { label: string; value: React.ReactNode; hint?: React.ReactNode; icon?: React.ReactNode; className?: string }) {
  return (
...
export function CountryFlag({ code, className }: { code: string; className?: string }) {
  ...
  return (
    <span className={cn('text-base leading-none', className)} role="img" aria-label={code.toUpperCase()}>
      {flag}
    </span>
  );
}
