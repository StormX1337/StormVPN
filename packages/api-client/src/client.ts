...
export type AdminNodeDetail = {
  node: AdminNodeDto;
  history: { at: string; cpuPercent: number; memoryPercent: number; loadPercent: number; activeConnections: number; rxBps: number; txBps: number }[];
};
...
export function createApiClient(options: HttpOptions = {}) {
  ...
  return {
    ...
    auth: {
      ...
      login: (input: LoginInput) => http.request<LoginResultDto>('POST', '/auth/login', { body: input, noRefresh: true }),
      loginMfa: (mfaToken: string, code: string) =>
        http.request<AuthResultDto>('POST', '/auth/login/mfa', { body: { mfaToken, code }, noRefresh: true }),
      logout: () => http.request<void>('POST', '/auth/logout', { body: {}, noRefresh: true }),
      verifyEmail: (token: string) => http.request<{ user: UserDto }>('POST', '/auth/verify-email', { body: { token }, noRefresh: true }),
      resendVerification: () => http.post<{ sent: boolean }>('/auth/resend-verification'),
      forgotPassword: (email: string) => http.request<{ sent: boolean }>('POST', '/auth/forgot-password', { body: { email }, noRefresh: true }),
      resetPassword: (token: string, password: string) =>
        http.request<void>('POST', '/auth/reset-password', { body: { token, password }, noRefresh: true }),
    },
    user: {
    ...
    account: {
      ...
      enableTwoFactor: (code: string) => http.post<{ backupCodes: string[] }>('/account/2fa/enable', { code }),
      disableTwoFactor: (password: string, code: string) => http.post<void>('/account/2fa/disable', { password, code }),
      regenerateBackupCodes: (code: string) => http.post<{ backupCodes: string[] }>('/account/2fa/backup-codes', { code }),
    ...
    devices: {
      ...
      rename: (deviceId: string, name: string) => http.patch<void>(`/devices/${id(deviceId)}`, { name }),
      ...
      revokePeer: (deviceId: string, peerId: string) => http.delete<void>(`/devices/${id(deviceId)}/peers/${id(peerId)}`),
    },
    servers: {
      list: (query: Partial<ListServersQuery> = {}) => http.get<ServerDto[]>('/servers', query as Query),
      get: (serverId: string) => http.get<ServerDto>(`/servers/${id(serverId)}`),
      recommended: (query: { country?: string; region?: string } = {}) => http.get<RecommendedServer>('/servers/recommended', query),
    ...
    connections: {
      ...
      disconnect: (connectionId: string) => http.delete<ConnectionDto>(`/connections/${id(connectionId)}`),
    },
    wireguard: {
      config: (input: WireGuardConfigInput) => http.post<WireGuardConfigDto & { selection: SelectionInfo }>('/wireguard/config', input),
    },
    traffic: {
    ...
    billing: {
      ...
      changePlan: (planId: string) => http.post<SubscriptionOverviewDto>('/billing/change-plan', { planId }),
      ...
      validateCoupon: (code: string, planId?: string) => http.post<CouponDto>('/billing/coupons/validate', { code, planId }),
    },
    admin: {
      ...
      suspendUser: (userId: string, reason: string) => http.post<void>(`/admin/users/${id(userId)}/suspend`, { reason }),
      unsuspendUser: (userId: string) => http.post<void>(`/admin/users/${id(userId)}/unsuspend`),
      setRole: (userId: string, role: Role) => http.patch<void>(`/admin/users/${id(userId)}/role`, { role }),
      revokeUserSessions: (userId: string) => http.post<{ revoked: number }>(`/admin/users/${id(userId)}/sessions/revoke`),
      disconnectUser: (userId: string) => http.post<{ disconnected: number }>(`/admin/users/${id(userId)}/disconnect`),
      grantPlan: (userId: string, planId: string, days?: number) => http.post<void>(`/admin/users/${id(userId)}/grant-plan`, { planId, days }),
      subscriptions: (query: Query = {}) => http.get<Paginated<AdminSubscriptionDto>>('/admin/subscriptions', query),
      ...
      syncSubscription: (subscriptionId: string) => http.post<void>(`/admin/subscriptions/${id(subscriptionId)}/sync`),
      ...
      updatePlan: (planId: string, input: PlanUpdateInput) => http.patch<AdminPlanDto>(`/admin/plans/${id(planId)}`, input),
      ...
      updateCoupon: (couponId: string, input: { isActive?: boolean; validUntil?: string | null; maxRedemptions?: number | null }) =>
      ...
      createServer: (input: ServerCreateInput) => http.post<AdminServerDto>('/admin/servers', input),
      updateServer: (serverId: string, input: ServerUpdateInput) => http.patch<AdminServerDto>(`/admin/servers/${id(serverId)}`, input),
      deleteServer: (serverId: string) => http.delete<void>(`/admin/servers/${id(serverId)}`),
      setServerStatus: (serverId: string, status: ServerStatus) => http.post<AdminServerDto>(`/admin/servers/${id(serverId)}/status`, { status }),
      setKillSwitch: (serverId: string, engaged: boolean, reason: string) =>
        http.post<AdminServerDto>(`/admin/servers/${id(serverId)}/kill-switch`, { engaged, reason }),
      enrollmentToken: (serverId: string) => http.post<EnrollmentTokenDto>(`/admin/servers/${id(serverId)}/enrollment-token`),
      ...
      connections: (query: Query = {}) => http.get<Paginated<AdminConnectionDto>>('/admin/connections', query),
      terminateConnection: (connectionId: string) => http.delete<void>(`/admin/connections/${id(connectionId)}`),
      traffic: (query: { from?: string; to?: string } = {}) => http.get<AdminTrafficDto>('/admin/traffic', query),
      logs: (query: Query = {}) => http.get<Paginated<AuditLogDto>>('/admin/logs', query),
      securityEvents: (query: Query = {}) => http.get<Paginated<AdminSecurityEventDto>>('/admin/security/events', query),
      resolveSecurityEvent: (eventId: string) => http.post<void>(`/admin/security/events/${id(eventId)}/resolve`),
      riskFlags: () => http.get<RiskFlagDto[]>('/admin/security/risk-flags'),
      createRiskFlag: (input: { subjectType: 'USER' | 'IP'; subjectValue: string; reason: string; score: number; expiresInHours?: number | null }) =>
        http.post<{ created: boolean }>('/admin/security/risk-flags', input),
      resolveRiskFlag: (flagId: string) => http.post<void>(`/admin/security/risk-flags/${id(flagId)}/resolve`),
      settings: () => http.get<SystemSettingsDto>('/admin/settings'),
      updateSettings: (input: SettingsUpdateInput) => http.patch<SystemSettingsDto>('/admin/settings', input),
    },
  };
}
