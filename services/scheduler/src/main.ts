...
async function main(): Promise<void> {
  ...
  const reconcile = async () => {
    ...
    logger.info(
      { jobs: schedule.map((entry) => `${entry.name}@${entry.everyMs / 1000}s`), removed },
      'maintenance schedule applied',
    );
  };
  await reconcile();
  const timer = setInterval(
    () =>
      void reconcile().catch((error: unknown) => logger.error({ err: error }, 'reconcile failed')),
