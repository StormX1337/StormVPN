...
describe('procfs parsers', () => {
  ...
  it('parses meminfo', () => {
    expect(parseMeminfo('MemTotal:  8000 kB\nMemFree: 100 kB\nMemAvailable:  2000 kB\n')).toEqual({ totalBytes: 8_192_000, availableBytes: 2_048_000 });
  });
...
describe('PeerTrafficTracker', () => {
  ...
  const peer = (rx: number, tx: number, handshake = now - 10) => ({ publicKey: 'A=', endpoint: null, allowedIps: [], latestHandshake: handshake, rxBytes: rx, txBytes: tx });

  it('reports deltas and handles counter resets', () => {
    const tracker = new PeerTrafficTracker();
    expect(tracker.collect([peer(100, 200)], now)).toEqual([{ publicKey: 'A=', latestHandshake: now - 10, rxBytesDelta: 100, txBytesDelta: 200 }]);
    expect(tracker.collect([peer(150, 260)], now)[0]).toMatchObject({ rxBytesDelta: 50, txBytesDelta: 60 });
    expect(tracker.collect([peer(20, 30)], now)[0]).toMatchObject({ rxBytesDelta: 20, txBytesDelta: 30 });
  });
  ...
  it('skips idle peers and re-sends deltas after a failed report', () => {
    ...
    expect(tracker.collect([peer(600, 600)], now)[0]).toMatchObject({ rxBytesDelta: 600, txBytesDelta: 600 });
  });
});
...
describe('misc', () => {
  ...
  it('detects public IPs with override and echo fallbacks', async () => {
    expect(await detectPublicIps({ ipv4Override: '203.0.113.5', echoUrls: [], timeoutMs: 100 })).toEqual({ ipv4: '203.0.113.5' });
    const fetchImpl = (async (url: string) =>
      url.includes('bad') ? new Response('<html>', { status: 200 }) : new Response('198.51.100.9\n')) as unknown as typeof fetch;
    expect(await detectPublicIps({ echoUrls: ['https://bad.test', 'https://good.test'], timeoutMs: 100, fetchImpl })).toEqual({ ipv4: '198.51.100.9' });
  });

  it('renders Prometheus metrics', () => {
    const text = renderMetrics(
      { up: true, appliedRevision: 7, peers: 3, activePeers: 2, cpuPercent: 1, memoryPercent: 2, diskPercent: 3, rxBps: 4, txBps: 5, lastHeartbeatSuccess: 6, heartbeatFailures: 0 },
      { server: 'DE-FRA-01' },
    );
