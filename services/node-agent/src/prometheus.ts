...
export function renderMetrics(
  ...
  return [
    ...
    metric(
      'stormvpn_agent_last_heartbeat_success_timestamp_seconds',
      'Last successful heartbeat',
      'gauge',
      snapshot.lastHeartbeatSuccess,
    ),
    metric(
      'stormvpn_agent_heartbeat_failures_total',
      'Failed heartbeats',
      'counter',
  ...
  return createServer((request, response) => {
    if (request.url === '/metrics') {
      response
        ...
        .end(renderMetrics(snapshot(), labels()));
