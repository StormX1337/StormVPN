...
function safe(value: string): string {
  if (!SAFE.test(value) || /[\r\n]/.test(value))
    throw new Error('Refusing unsafe value in WireGuard config');
  return value;
}

/** `wg(8)` format used by `wg syncconf` (interface private key + peers). */
export function renderSyncConfig(
  privateKey: string,
  listenPort: number,
 ...
 */
export function renderInterfaceConfig(
  privateKey: string,
  config: AgentConfigResponse['interface'],
): string {
