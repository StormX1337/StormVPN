...
function ServerFormDialog({
  ...
  const save = useAction(
    (input: Record<string, unknown>) =>
      server
        ? api.admin.updateServer(server.id, input as never)
        : api.admin.createServer(input as never),
    {
      success: server ? 'Server updated' : 'Server created',
      invalidate,
      ...
      <DialogContent className="max-w-2xl">
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              After creating the server, issue an enrollment token and run the node installer on the
              host.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Hostname" htmlFor="hostname" className="sm:col-span-2">
              <Input
                id="hostname"
                name="hostname"
                placeholder="de-fra-03.nodes.example.com"
            ...
            <Field label="WireGuard port" htmlFor="wireguardPort">
              <Input
                ...
                name="wireguardPort"
                type="number"
                defaultValue={server?.wireguardPort ?? 51820}
              />
            </Field>
            <Field label="Class" htmlFor="serverClass">
              <NativeSelect
                id="serverClass"
                name="serverClass"
                defaultValue={server?.serverClass ?? 'STANDARD'}
              >
            ...
            <Field label="Max connections" htmlFor="capacity">
              <Input
                ...
                name="capacity"
                type="number"
                defaultValue={server?.capacity ?? 500}
              />
            </Field>
            <Field label="Bandwidth (Mbit/s)" htmlFor="bandwidthCapacityMbps">
              <Input
                id="bandwidthCapacityMbps"
                name="bandwidthCapacityMbps"
                type="number"
                defaultValue={server?.bandwidthCapacityMbps ?? 1000}
              />
            </Field>
            <Field label="Client subnet IPv4" htmlFor="wgSubnetV4" hint="Unique per server">
              <Input
                id="wgSubnetV4"
                name="wgSubnetV4"
                defaultValue={server?.wgSubnetV4 ?? '10.80.0.0/20'}
              />
            </Field>
            <Field label="Client subnet IPv6" htmlFor="wgSubnetV6">
              <Input
                id="wgSubnetV6"
                name="wgSubnetV6"
                placeholder="fd80:1::/64"
                defaultValue={server?.wgSubnetV6 ?? ''}
              />
            </Field>
            <Field label="DNS override" htmlFor="dnsServers" hint="Empty = node resolver">
              <Input
                ...
                defaultValue={server?.dnsServers.join(', ')}
              />
            </Field>
...
export function ServersView() {
  ...
  const status = useAction(
    ({ id, value }: { id: string; value: 'ACTIVE' | 'DISABLED' | 'MAINTENANCE' }) =>
      api.admin.setServerStatus(id, value),
    { success: 'Status updated', invalidate },
  );
  const remove = useAction((id: string) => api.admin.deleteServer(id), {
    success: 'Server removed',
    invalidate,
    onDone: () => setDeleting(null),
  });
  const kill = useAction(
    ({ id, engaged }: { id: string; engaged: boolean }) =>
      api.admin.setKillSwitch(id, engaged, killReason || 'released'),
    {
  ...
  return (
    <>
            ...
            {servers.map((server) => (
              <TableRow key={server.id}>
                <TableCell>
                  ...
                  <div className="text-muted-foreground text-xs">{server.hostname}</div>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  ...
                  {server.killSwitchEngaged ? (
                    <StatusBadge className="ml-1" tone="critical" label="Kill switch" />
                  ) : null}
                </TableCell>
                <TableCell>
                  {server.nodeStatus ? (
                    <StatusBadge status={server.nodeStatus} />
                  ) : (
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        onSelect={() => status.mutate({ id: server.id, value: 'ACTIVE' })}
                      >
                        <Power /> Activate
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onSelect={() => status.mutate({ id: server.id, value: 'MAINTENANCE' })}
                      >
                        <Wrench /> Maintenance
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onSelect={() => status.mutate({ id: server.id, value: 'DISABLED' })}
                      >
                        Disable
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      {server.killSwitchEngaged ? (
                        <DropdownMenuItem
                          onSelect={() => kill.mutate({ id: server.id, engaged: false })}
                        >
                          Release kill switch
                        </DropdownMenuItem>
                      ) : (
      ...
      <ServerFormDialog
        ...
        onOpenChange={(open) => !open && (setCreating(false), setEditing(null))}
      />
      <ConfirmDialog
        open={deleting !== null}
      ...
      <Dialog open={enrollment !== null} onOpenChange={(open) => !open && setEnrollment(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Single use, expires {formatDateTime(enrollment?.expiresAt)}. Run on the new node as
              root:
            </DialogDescription>
          </DialogHeader>
          <pre className="bg-muted/60 overflow-x-auto rounded-lg border p-3 font-mono text-xs break-all whitespace-pre-wrap">
