...
function PlanDialog({
  ...
  const save = useAction(
    (input: Record<string, unknown>) =>
      plan ? api.admin.updatePlan(plan.id, input) : api.admin.createPlan(input as never),
    {
      ...
      <DialogContent className="max-w-2xl">
        <form
          ...
          onSubmit={(event) => {
            ...
            const form = Object.fromEntries(new FormData(event.currentTarget).entries()) as Record<
              ...
              string
            >;
            const classes = Object.values(ServerClass).filter(
              (value) => form[`class_${value}`] === 'on',
            );
            const input = {
              slug: form.slug,
              name: form.name,
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Price changes create a new Stripe price; existing subscribers keep their current
              price.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Price" htmlFor="price">
              <Input
                ...
                step="0.01"
                min="0"
                defaultValue={plan ? plan.priceCents / 100 : 0}
              />
            </Field>
            ...
            <Field label="Max devices" htmlFor="maxDevices">
              <Input
                ...
                type="number"
                min={1}
                defaultValue={plan?.maxDevices ?? 3}
              />
            </Field>
            <Field label="Max simultaneous" htmlFor="maxSessions">
              <Input
                ...
                type="number"
                min={1}
                defaultValue={plan?.maxSessions ?? 3}
              />
            </Field>
            <Field label="Traffic / month (GB)" htmlFor="trafficGb" hint="Empty = unlimited">
              <Input
                ...
                type="number"
                min={0}
                defaultValue={plan?.trafficLimitBytes ? plan.trafficLimitBytes / GB : ''}
              />
            </Field>
            <Field
              label="Allowed countries"
              htmlFor="allowedCountries"
              hint="Comma separated ISO codes, empty = all"
            >
              <Input
                id="allowedCountries"
                name="allowedCountries"
                defaultValue={plan?.allowedCountries.join(', ')}
              />
            </Field>
                ...
                {Object.values(ServerClass).map((value) => (
                  <label key={value} className="flex items-center gap-2">
                    <input
                      ...
                      name={`class_${value}`}
                      defaultChecked={
                        plan ? plan.serverClasses.includes(value) : value === 'STANDARD'
                      }
...
export function PlansView() {
  ...
  const remove = useAction((id: string) => api.admin.deletePlan(id), {
            ...
            {plans.map((plan) => (
              <TableRow key={plan.id}>
                <TableCell>
                  ...
                  <div className="text-muted-foreground font-mono text-xs">{plan.slug}</div>
                </TableCell>
                <TableCell className="tabular">
                  {plan.isFree
                    ? 'Free'
                    : `${formatCurrency(plan.priceCents, plan.currency)} / ${plan.intervalCount > 1 ? `${plan.intervalCount} ` : ''}${plan.billingInterval.toLowerCase()}`}
                  {plan.trialDays ? (
                    <div className="text-muted-foreground text-xs">{plan.trialDays}-day trial</div>
                  ) : null}
                </TableCell>
                <TableCell className="text-muted-foreground text-xs">
                  {plan.maxDevices} devices · {plan.maxSessions} sessions ·{' '}
                  {plan.trafficLimitBytes === null
                    ? 'unlimited'
                    : formatBytes(plan.trafficLimitBytes, 0)}
                  <div>
                    {plan.allowedCountries.length
                      ? plan.allowedCountries.join(', ')
                      : 'all countries'}
                  </div>
                </TableCell>
                <TableCell className="text-xs">
                  {plan.serverClasses.map((value) => value.toLowerCase()).join(', ')}
                </TableCell>
                ...
                <TableCell>
                  <StatusBadge
                    tone={plan.isActive ? 'good' : 'neutral'}
                    label={plan.isActive ? (plan.isPublic ? 'Public' : 'Hidden') : 'Inactive'}
                  />
                </TableCell>
...
export function CouponsView() {
  ...
  const create = useAction(
    (input: Record<string, unknown>) => api.admin.createCoupon(input as never),
    {
  ...
  const toggle = useAction(
    ({ id, isActive }: { id: string; isActive: boolean }) =>
      api.admin.updateCoupon(id, { isActive }),
    { invalidate: [['admin', 'coupons']] },
  );
  return (
    <>
            ...
            {coupons.map((coupon) => (
              <TableRow key={coupon.id}>
                <TableCell>
                  ...
                  <div className="text-muted-foreground text-xs">{coupon.name}</div>
                </TableCell>
                <TableCell>
                  {coupon.percentOff
                    ? `${coupon.percentOff}%`
                    : formatCurrency(coupon.amountOffCents ?? 0, coupon.currency ?? 'eur')}
                </TableCell>
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              const input = {
                ...
                durationInMonths:
                  form.duration === 'REPEATING' ? Number(form.durationInMonths) : null,
