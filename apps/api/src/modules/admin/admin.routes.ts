...
function actorOf(request: FastifyRequest): ActorContext {
  return { actorId: authOf(request).userId, actorType: 'ADMIN', ipAddress: clientIp(request), userAgent: userAgent(request) };
}
...
export async function adminRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  app.addHook('preHandler', async (request) => {
    if (request.method !== 'GET' && authOf(request).role !== 'ADMIN') throw forbidden('read_only', 'Support staff have read-only access');
  });
  ...
  // Users
  app.get('/users', { schema: { querystring: adminUsersQuerySchema } }, async (request) => adminUsers.list(request.query));
  app.get('/users/:id', params, async (request) => adminUsers.detail(request.params.id));
  app.post('/users/:id/suspend', { schema: { params: idParamsSchema, body: suspendUserSchema } }, async (request, reply) => {
  ...
  app.patch('/users/:id/role', { schema: { params: idParamsSchema, body: updateUserRoleSchema } }, async (request, reply) => {
  ...
  app.post('/users/:id/grant-plan', { schema: { params: idParamsSchema, body: adminGrantPlanSchema } }, async (request, reply) => {
    await adminUsers.grantPlan(request.params.id, request.body.planId, request.body.days, actorOf(request));
    return reply.status(204).send();
  });

  // Subscriptions & payments
  app.get('/subscriptions', { schema: { querystring: adminSubscriptionsQuerySchema } }, async (request) =>
    adminBilling.listSubscriptions(request.query),
  );
  app.post(
    '/subscriptions/:id/cancel',
    { schema: { params: idParamsSchema, body: z.object({ immediately: z.boolean().default(false) }) } },
    async (request, reply) => {
      await adminBilling.cancelSubscription(request.params.id, request.body.immediately, actorOf(request));
      return reply.status(204).send();
    },
  );
  ...
  app.get('/payments', { schema: { querystring: adminPaymentsQuerySchema } }, async (request) => adminBilling.listPayments(request.query));
  ...
  app.patch('/plans/:id', { schema: { params: idParamsSchema, body: planUpdateSchema } }, async (request) =>
    adminBilling.updatePlan(request.params.id, request.body, actorOf(request)),
  );
  ...
  app.patch('/coupons/:id', { schema: { params: idParamsSchema, body: couponUpdateSchema } }, async (request) =>
    adminBilling.updateCoupon(request.params.id, request.body, actorOf(request)),
  );
  ...
  app.patch('/servers/:id', { schema: { params: idParamsSchema, body: serverUpdateSchema } }, async (request) =>
    adminInfra.updateServer(request.params.id, request.body, actorOf(request)),
  );
  app.delete('/servers/:id', params, async (request, reply) => {
    await adminInfra.deleteServer(request.params.id, actorOf(request));
    return reply.status(204).send();
  });
  app.post('/servers/:id/status', { schema: { params: idParamsSchema, body: serverStatusSchema } }, async (request) =>
    adminInfra.setServerStatus(request.params.id, request.body.status, actorOf(request)),
  );
  app.post('/servers/:id/kill-switch', { schema: { params: idParamsSchema, body: killSwitchSchema } }, async (request) =>
    adminInfra.setKillSwitch(request.params.id, request.body.engaged, request.body.reason, actorOf(request)),
  );
  ...
  // Connections & traffic
  app.get('/connections', { schema: { querystring: adminConnectionsQuerySchema } }, async (request) =>
    adminInfra.listConnections(request.query),
  );
  ...
  app.get('/logs', { schema: { querystring: auditLogsQuerySchema } }, async (request) => adminSecurity.auditLogs(request.query));
  app.get('/security/events', { schema: { querystring: securityEventsQuerySchema } }, async (request) =>
    adminSecurity.securityEvents(request.query),
  );
  ...
  app.post('/security/risk-flags', { schema: { body: riskFlagCreateSchema } }, async (request, reply) => {
