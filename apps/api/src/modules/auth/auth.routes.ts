...
export async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  app.get('/csrf', async (request, reply) => ({ csrfToken: ensureCsrfCookie(request, reply, env) }));

  app.post('/register', { config: strict, schema: { body: registerSchema } }, async (request, reply) => {
  ...
  app.post('/login', { config: strict, schema: { body: loginSchema } }, async (request, reply): Promise<LoginResultDto> => {
  ...
  app.post('/login/mfa', { config: strict, schema: { body: mfaLoginSchema } }, async (request, reply) => {
    const issued = await auth.completeMfaLogin(request.body.mfaToken, request.body.code, metaOf(request));
    return respondWithSession(request, reply, env, issued);
  });

  app.post('/refresh', { config: strict, schema: { body: refreshSchema.nullish() } }, async (request, reply) => {
    const token = request.body?.refreshToken ?? request.cookies[COOKIE_REFRESH_TOKEN];
    if (!token) throw unauthorized('missing_refresh_token', 'Session expired, please sign in again');
    try {
      const issued = await sessions.rotate(token, { ipAddress: clientIp(request), userAgent: userAgent(request) });
      return respondWithSession(request, reply, env, issued);
    } catch (error) {
  ...
  app.post('/resend-verification', { preHandler: [app.authenticate], config: strict }, async (request, reply) => {
    await auth.resendVerification(authOf(request).userId);
    return reply.status(202).send({ sent: true });
  });

  app.post('/forgot-password', { config: strict, schema: { body: forgotPasswordSchema } }, async (request, reply) => {
    await auth.forgotPassword(request.body.email, metaOf(request));
    return reply.status(202).send({ sent: true });
  });

  app.post('/reset-password', { config: strict, schema: { body: resetPasswordSchema } }, async (request, reply) => {
