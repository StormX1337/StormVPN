...
export interface SelectionContext {
  ...
  preferences: { preferredCountry?: string | null; preferredRegion?: string | null; favoriteServerIds: Set<string> };
...
export function ineligibility(candidate: Candidate, ctx: SelectionContext): IneligibleReason | null {
  if (candidate.status !== 'ONLINE' && candidate.status !== 'DEGRADED') return 'offline';
  if (ctx.allowedCountries.length > 0 && !ctx.allowedCountries.includes(candidate.countryCode)) return 'plan_country';
...
export function latencyFor(candidate: Candidate, ctx: SelectionContext): number | null {
  ...
  if (ctx.clientLocation && candidate.latitude !== null && candidate.longitude !== null) {
    return estimateLatencyMs(haversineKm(ctx.clientLocation, { lat: candidate.latitude, lon: candidate.longitude }));
  }
  return null;
}
...
export function scoreCandidate(candidate: Candidate, ctx: SelectionContext): ScoredCandidate {
  ...
  const headroom = candidate.capacity > 0 ? ((candidate.capacity - candidate.activeConnections) / candidate.capacity) * 100 : 0;

  let score = WEIGHTS.load * loadScore + WEIGHTS.latency * latencyScore + WEIGHTS.headroom * headroom;
  if (ctx.preferences.favoriteServerIds.has(candidate.id)) score += 8;
  if (ctx.preferences.preferredCountry && candidate.countryCode === ctx.preferences.preferredCountry) score += 10;
  if (ctx.preferences.preferredRegion && candidate.region === ctx.preferences.preferredRegion) score += 4;

  const softLimit = ctx.overloadThreshold - PRIORITY_RESERVE_MARGIN;
  if (ctx.planPriority < HIGH_PRIORITY && candidate.load > softLimit) score -= (candidate.load - softLimit) * 2;
...
export function selectServer(
  ...
  const ranked = pool.map((candidate) => scoreCandidate(candidate, ctx)).sort((a, b) => b.score - a.score);
