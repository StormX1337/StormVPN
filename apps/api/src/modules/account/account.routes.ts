...
export async function accountRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  app.post('/password', { config: sensitive, schema: { body: changePasswordSchema } }, async (request, reply) => {
  ...
  app.post('/2fa/setup', async (request) => mfa.beginSetup(await account.getUser(authOf(request).userId)));

  app.post('/2fa/enable', { config: sensitive, schema: { body: enableTwoFactorSchema } }, async (request) => {
    const user = await account.getUser(authOf(request).userId);
    return { backupCodes: await mfa.enable(user, request.body.code) };
  });

  app.post('/2fa/disable', { config: sensitive, schema: { body: disableTwoFactorSchema } }, async (request, reply) => {
    const user = await account.getUser(authOf(request).userId);
    await account.assertPassword(user, request.body.password);
    if (!(await mfa.verify(user, request.body.code))) {
      await recordSecurityEvent(app.deps.db, { userId: user.id, type: 'MFA_FAILED', ...requestContext(request) });
  ...
  app.post('/2fa/backup-codes', { config: sensitive, schema: { body: regenerateBackupCodesSchema } }, async (request) => {
    const user = await account.getUser(authOf(request).userId);
    if (!(await mfa.verify(user, request.body.code))) throw badRequest('invalid_mfa_code', 'Invalid verification code');
  ...
  app.get('/security-events', async (request) => account.listSecurityEvents(authOf(request).userId));

  app.delete('/', { config: sensitive, schema: { body: deleteAccountSchema } }, async (request, reply) => {
    await account.deleteAccount(authOf(request).userId, request.body.password, requestContext(request));
