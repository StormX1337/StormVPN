 ...
 */
export const nodeAuthPlugin = fp(
  async (app: FastifyInstance, opts: { db: Database; redis: Redis }) => {
    app.decorate('authenticateNode', async (request: FastifyRequest) => {
      ...
      if (failures >= FAILURE_LIMIT)
        throw tooManyRequests('node_auth_throttled', 'Too many failed node authentications');

      const header = request.headers.authorization;
      const token = header?.startsWith('Bearer snt_') ? header.slice(7).trim() : null;
      const node = token
        ? await opts.db.vPNNode.findUnique({
            where: { tokenHash: sha256Hex(token) },
            select: { id: true, serverId: true },
          })
        : null;
      if (!node) {
        const count = await opts.redis
          .multi()
          .incr(failureKey)
          .expire(failureKey, FAILURE_WINDOW_SECONDS)
          .exec();
