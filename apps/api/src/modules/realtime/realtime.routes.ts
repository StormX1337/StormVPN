...
export async function realtimeRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/ws', { websocket: true, preHandler: [app.authenticate], config: { rateLimit: { max: 30, timeWindow: '1 minute' } } }, (socket, request) => {
    ...
    socket.on('message', (raw: Buffer) => {
      ...
      try {
        ...
        if (message.type === 'ping') socket.send(JSON.stringify({ type: 'pong', data: { ts: Date.now() } }));
      } catch {
