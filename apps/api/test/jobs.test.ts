...
describe('maintenance jobs', () => {
      ...
      data: [
        {
          ...
          serverId: server.id,
          status: 'CONNECTED',
          lastHandshakeAt: new Date(Date.now() - 20 * 60_000),
        },
        {
          userId: session.userId,
          serverId: server.id,
  ...
  it('disables peers when the traffic allowance is exhausted and restores them after an upgrade', async () => {
    ...
    await h.db.trafficUsage.create({
      data: {
        ...
        serverId: server.id,
        day: new Date(),
        rxBytes: BigInt(6 * 1024 ** 3),
        txBytes: BigInt(5 * 1024 ** 3),
      },
    });
    expect((await runTrafficEnforcement(ctx)).limited).toBe(1);
    expect(await h.db.vPNPeer.findFirstOrThrow()).toMatchObject({
      status: 'DISABLED',
      disabledReason: 'TRAFFIC_LIMIT',
    });
    ...
    await h.db.subscription.create({
      data: { userId: session.userId, planId: pro.id, status: 'ACTIVE' },
    });
  ...
  it('automatically suspends accounts with a high risk score', async () => {
    ...
    expect(
      await h.db.auditLog.count({ where: { action: 'user.suspend', actorType: 'SYSTEM' } }),
    ).toBe(1);
  });
});
