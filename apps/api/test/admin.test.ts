...
describe('admin authorization', () => {
  it('denies regular users, allows support read-only and admins full access', async () => {
    ...
    expect((await call(h, null, { method: 'GET', url: '/api/v1/admin/stats' })).statusCode).toBe(
      401,
    );
    expect((await call(h, user, { method: 'GET', url: '/api/v1/admin/stats' })).statusCode).toBe(
      403,
    );
    expect((await call(h, user, { method: 'GET', url: '/api/v1/admin/users' })).statusCode).toBe(
      403,
    );

    expect((await call(h, support, { method: 'GET', url: '/api/v1/admin/users' })).statusCode).toBe(
      200,
    );
    const supportWrite = await call(h, support, {
      method: 'POST',
    ...
    const created = await call(h, admin, {
      method: 'POST',
      url: '/api/v1/admin/servers',
      payload: serverPayload,
    });
  ...
  it('takes role changes into account immediately', async () => {
    ...
    expect(
      (await call(h, promoted, { method: 'GET', url: '/api/v1/admin/stats' })).statusCode,
    ).toBe(403);
    const change = await call(h, admin, {
      method: 'PATCH',
      url: `/api/v1/admin/users/${promoted.userId}/role`,
      payload: { role: 'ADMIN' },
    });
    ...
    const self = await call(h, admin, {
      method: 'PATCH',
      url: `/api/v1/admin/users/${admin.userId}/role`,
      payload: { role: 'USER' },
    });
...
describe('admin operations', () => {
  it('suspends/unsuspends users with audit trail', async () => {
    ...
    expect((await h.db.user.findUniqueOrThrow({ where: { id: user.userId } })).status).toBe(
      'SUSPENDED',
    );
  ...
  it('manages plans dynamically and syncs paid plans to Stripe', async () => {
    ...
    const create = await call(h, admin, {
      ...
      payload: {
        ...
        maxDevices: 10,
        maxSessions: 10,
        serverClasses: ['STANDARD', 'PREMIUM', 'STREAMING'],
      },
    });
    ...
    const update = await call(h, admin, {
      method: 'PATCH',
      url: `/api/v1/admin/plans/${plan.id}`,
      payload: { priceCents: 1599 },
    });
    ...
    expect(publicPlans.json().map((p: { slug: string }) => p.slug)).toEqual([
      'free',
      'pro',
      'ultra',
    ]);
  ...
  it('creates coupons, rejects overlapping subnets and toggles maintenance mode', async () => {
    const admin = await registerUser(h, 'admin@example.com', { role: 'ADMIN' });
    const coupon = await call(h, admin, {
      method: 'POST',
      url: '/api/v1/admin/coupons',
      payload: { code: 'storm20', percentOff: 20 },
    });
    ...
    const overlap = await call(h, admin, {
      method: 'POST',
      url: '/api/v1/admin/servers',
      payload: serverPayload,
    });
    ...
    const settings = await call(h, admin, {
      method: 'PATCH',
